...
import { MCQS, type MCQ } from "@/lib/soc-prep/mcq";
import { type Level } from "@/lib/soc-prep/data";
import { LevelBadge } from "./parts";
import { ListenButton, plainText, speech } from "./speech";

type LevelFilter = "all" | Level;
type Phase = "config" | "active" | "done";

const LETTERS = ["A", "B", "C", "D", "E"];
const QUIZ_CATS = [...new Set(MCQS.map((q) => q.category))];
...
/** In-browser Fisher–Yates shuffle (returns a new array). */
...
export function SocQuiz({ level }: { level: LevelFilter }) {
  const [cat, setCat] = useState("all");
  const [shuffle, setShuffle] = useState(true);
  const [phase, setPhase] = useState<Phase>("config");
  const [deck, setDeck] = useState<MCQ[]>([]);
  ...
  const pool = useMemo(
    () =>
      MCQS.filter(
        (q) => (level === "all" || q.level === level) && (cat === "all" || q.category === cat)
      ),
    [level, cat]
  );
  ...
  const catCounts = useMemo(
    () =>
      Object.fromEntries(
        QUIZ_CATS.map((c) => [
          c,
          MCQS.filter((q) => c === q.category && (level === "all" || q.level === level)).length,
        ])
      ),
    [level]
  );

  const begin = (questions: MCQ[]) => {
    speech.stop();
  ...
  // Keyboard: 1–4 / A–D to answer, Enter or → to advance. Ignored while typing.
  useEffect(() => {
    ...
    return (
      <div ref={cardRef} className="soc-card rounded-lg border-[1.5px] border-ink bg-surface p-5 sm:p-7">
        <p className="soc-prose max-w-(--soc-measure)">
          Multiple-choice drill across the same tiers and topics as the kit. Pick a
          topic, answer, and get the reasoning immediately. Use{" "}
          <b>the current tier filter</b> above to narrow difficulty.
        </p>
        ...
        {pool.length === 0 && (
          <p className="mono-label mt-4 text-muted-2">
            No questions for this filter — widen the tier or topic.
          </p>
        )}
      </div>
    );
  ...
  if (phase === "done") {
    ...
    const verdict =
      pct >= 90 ? "Interview-ready." : pct >= 70 ? "Solid — tighten the misses." : "Keep drilling.";

    // Per-topic breakdown for the questions actually attempted.
    const byCat = QUIZ_CATS.map((c) => {
      const items = deck.filter((d) => d.category === c);
      if (!items.length) return null;
      const got = items.filter((d) => picks[d.id] === d.answer).length;
